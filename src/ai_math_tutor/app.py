...
# create data and chroma_db directories if they do not already exist
...
SQLITE_DB_PATH = os.path.join(str(PROJECT_ROOT), "data", "book_content.db")



from ai_math_tutor.backend import end_to_end_pipeline, RagApplication

# --- Page Configuration ---
st.set_page_config(
    page_title="AI Mathematics Tutor",
    page_icon="📖",
    layout="wide"
)
load_dotenv()
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        if st.button("Process Textbook"):
            # ... (File saving and ingestion logic is the same) ...
            with st.spinner("Processing..."):
                file_path = os.path.join(str(PROJECT_ROOT), "data", uploaded_file.name)
                with open(file_path, "wb") as f:
                    f.write(uploaded_file.getbuffer())
                
                rag_app = RagApplication(collection_name=SAMPLE_BOOK, content_db_path=SQLITE_DB_PATH)  #end_to_end_pipeline(file_path)
...
if st.session_state.processed_file:
    ...
    with col2:
        ...
        current_page_history = st.session_state.page_chats.get(st.session_state.page_num, [])
        for message in current_page_history:
            role = "user" if isinstance(message, HumanMessage) else "assistant"
            with st.chat_message(role):
                st.markdown(message.content)

        if prompt := st.chat_input(f"Ask a question about page {st.session_state.page_num}..."):
            st.session_state.page_chats.setdefault(st.session_state.page_num, [])

            st.session_state.page_chats[st.session_state.page_num].append(HumanMessage(content=prompt))
            ...
            st.session_state.page_chats[st.session_state.page_num].append(AIMessage(content=response))
else:
